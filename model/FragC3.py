...
def reverse_edge(tensor: torch.Tensor):
    ...
    n = tensor.size(0)
    if n == 0:
        return tensor  # Return as-is for an empty tensor
...
class MVMP(nn.Module):
    def __init__(
        ...
        view: a: atom, p: frag, j: use junction-enhanced membership edges
              e.g., "apj" means using three channels: a-b-a, p-r-p, and a-c-p / p-c-a (with f_junc_*).
        """
        ...
        # Only two node types exist: atoms ("a") and fragments ("p")
        self.node_types = ["a", "p"]

        # Homogeneous edges: atom-atom; if 'p' is included, also add fragment-fragment
        self.homo_etypes = [("a", "b", "a")]
        if "p" in view:
            self.homo_etypes.append(("p", "r", "p"))

        # Heterogeneous membership edges: use the existing 'c' relation from graph preparation
        # Previously mistyped as 'j', which caused DGL to report a missing edge type
        self.hetero_etypes = []
        if "j" in view:
            self.hetero_etypes = [("a", "c", "p"), ("p", "c", "a")]

        # Attention blocks (one per edge type)
        self.attn = nn.ModuleDict()
        for etype in self.homo_etypes + self.hetero_etypes:
            self.attn["".join(etype)] = MultiHeadedAttention(heads, hid_dim)

        # Intermediate layers for homogeneous edges
        ...
        # Final aggregation layers (by destination node type)
    ...
    def update_node(self, node, field, layer):
        # Concatenate received mailbox messages with the current node state and static features
        return {
            field: layer(
    ...
    def forward(self, bg):
        ...
        # Initialize cached node features and edge features
        for ntype in self.node_types:
            bg.apply_nodes(self.init_node, ntype=ntype)
        for etype in self.homo_etypes:
            bg.apply_edges(self.init_edge, etype=etype)

        # If the 'j' view is enabled, store junction-enhanced features into f_junc_{suffix}
        if "j" in self.view:
        ...
        # --- Stage 1: multi-layer message passing on homogeneous edges ---
        update_funcs_homo = {
            e: (
                fn.copy_e("h", "m"),
        ...
        for i in range(self.depth - 1):
            ...
            # Pair reverse edges and update edge states
            for edge_type in self.homo_etypes:
        ...
        # --- Stage 2: aggregate homogeneous-edge messages into nodes (a/p use their own last_layer) ---
        update_funcs_last = {
            e: (
                fn.copy_e("h", "mail"),
        ...
        # --- Stage 3: if 'j' view is enabled, propagate over heterogeneous membership edges (use f_junc_* as message source) ---
...
# =========================
# FragC3: multi-view fragment encoder
# =========================
class FragC3(nn.Module):
    """
    Drug-combination prediction model with multi-view fragment fusion.

    Args:
      n_output: output dimension (classification or regression)
      cell_dim: input dimensionality of raw cell-line features
      hid_dim: hidden dimension
      heads / ffn_expansion / dropout: same meaning as in the existing implementation
      tri_attn: whether to enable cell-conditioned cross-context attention in Bi2Frag
      frag_list: fragment views to use, e.g., any subset of ["brics","fg","murcko","ringpaths"]
      frag_agg: view aggregation strategy: 'mlp' / 'gate' / 'tri_attn'
    """
    ...
    def __init__(self, n_output=2, cell_dim=1722, hid_dim=300, heads=6, ffn_expansion=4, use_C3Attn=False,
        ...
        # Normalize and validate fragment view list
        ...
        # (1) Per-view input projection layers
        # Dimensions follow the original implementation
        ...
        # Fragment-level readout and GRU (shared across views; node-level output dims match)
        ...
        # (4) Cross-view aggregation head
        self.frag_agg = frag_agg
        out_per_drug = hid_dim * 2  # Bidirectional GRU output dim (after pooling)
        ...
        if frag_agg == "mlp":
            ...
            # Learnable gating (scalar gate per view, then softmax)
            self.gate = nn.Linear(out_per_drug, 1)
        elif frag_agg == "cell_attn":
            # Use the cell vector as Query and attend over view embeddings as Keys
        ...
        # (5) Final prediction head: concat(drugA, drugB, cell)
    ...
    # Backward compatibility with the legacy name "MultiSyn" (optional)
    # You can also directly use FragC3 in training scripts and set frag_list=["brics"] to reproduce legacy behavior
    # (Keeping an alias like `class MultiSyn = FragC3` is also possible; see the end of the file.)
    ...
    # Use the per-view independent input projection layers
    def init_feature_for_view(self, bg, view_key: str):
        ...
        # Bond features ("x") usually exist; keep a small fallback for edge cases
        ...
        # p-r-p edges: robustly fetch/initialize "x" (key fallback path)
        ...
        if not x_exist:
            ...
            if xr.dim() == 1:
                xr = xr.view(-1, 34)
            # Some graph builders may write empty shapes; fall back to (E, 34)
        ...
        # Fragment node features and junction-enhanced features
        bg.nodes["p"].data["f"] = self.act(proj["w_pharm"](bg.nodes["p"].data["f"]))

        # Junction features (concatenated f_junc is expected to exist; apply a linear projection only)
    ...
    def _encode_one_view(self, graphs_a, graphs_b, view_key, cell_vec=None, return_attn=False, frag_masks=None):
        # batch & init
        ga = dgl.batch(graphs_a);
        gb = dgl.batch(graphs_b)
        self.init_feature_for_view(ga, view_key)
        self.init_feature_for_view(gb, view_key)

        self.mvmp[view_key](ga);
        self.mvmp[view_key](gb)
        ...
        def pool_valid(h, sizes):
            ...
            for i in range(h.shape[0]):
                n = int(sizes[i]);
                embs.append(h[i, :n].reshape(n, -1).mean(0, keepdim=True))
            return torch.cat(embs, dim=0)  # [B, 2*D]

        emb_a = pool_valid(h_a, node_size_a)
        emb_b = pool_valid(h_b, node_size_b)

        # If return_attn=True, also include valid_a and valid_b in the returned dict
        if return_attn and attn is not None:
            # attn is already in the form {"B2A": ..., "A2B": ...}
            # Attach valid_a and valid_b
            attn = {
                "attn": attn,
                "valid_a": valid_a,
                "valid_b": valid_b
    ...
    def forward(self, data, return_attn: bool = False):
        ...
        # Prepare per-view graph lists (provided by dataset.py fields)
        ...
        # Encode each view → obtain a [B, 2D] embedding per drug
        embsA = [];
        embsB = [];
        extra_per_view = {}
        ...
        for k in self.frag_list:
            ...
            emb_a, emb_b, extra = self._encode_one_view(ga, gb, k, cell_vec=cell if self.tri_attn else None,
                                                        return_attn=return_attn, frag_masks=frag_masks)
            embsA.append(emb_a);
            embsB.append(emb_b)
