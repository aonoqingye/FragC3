...
from datas.prepare_oneil_data_efgs import GetFragmentFeats, _get_mol_fragment_sets_fg, _get_mol_fragment_sets_murcko
...
def extract_target_and_pred(y_i: torch.Tensor, out_i: torch.Tensor) -> Tuple[float, float, float, float]:
    """
    修正后的指标提取：自动识别分类与回归任务。
    """
    y_true = float(y_i.detach().cpu().item() if y_i.numel() == 1 else y_i.detach().cpu().squeeze().item())

    # 判定是否为分类任务 (输出维度 > 1 通常为分类)
    is_classification = (out_i.dim() > 0 and out_i.shape[-1] > 1)

    if is_classification:
        # 分类任务：取正类(index 1)的概率
        probs = F.softmax(out_i.detach().cpu(), dim=-1)
        y_pred = float(probs[1].item() if probs.dim() == 1 else probs[0, 1].item())

        abs_err = abs(y_pred - y_true)
        # 【修正】分类任务相对误差无意义且危险，强制设为与 abs_err 一致或 0
        rel_err = abs_err
    else:
        # 回归任务
        y_pred = float(out_i.detach().cpu().item() if out_i.numel() == 1 else out_i.detach().cpu().squeeze().item())
        abs_err = abs(y_pred - y_true)
        # 仅在回归且 y_true 非 0 时计算相对误差
...
def save_cross_attn_pairlines(
    ...
    # === New: structured export for case study ===
    write_sample_json: bool = True,
    minimal_text: Optional[bool] = None,
    sample_json_suffix: str = "sample.json",
    export_top_pairs_k: int = 2,
    export_top_edges_k: int = None,
    export_top_frag_k: int = 3,
    ...
    samples_out = [] if return_samples else None
    if export_top_edges_k is None:
        export_top_edges_k = top_k
    ...
    def _arrow(ax, x1, y1, x2, y2, color, wn, offset_sign):
        ...
        ax.text(midx, midy, f"{wn:.2f}", fontsize=10, color=color, alpha=0.85, ha="center")
    ...
    for i in range(n):
        ...
        def draw_one_view(i: int, row_idx: int, view_key: str):
            ...
            ax_title.text(0.0, 0.5, f"View: {view_disp}", fontsize=12, va="center", ha="left",
            ...
            # —— Assemble structured evidence for this view (pairs/fragments/weights) ——
            view_index = view_order.index(view_key)
            weightA = float(agg_attn['A'][i, 0, view_index])
            weightB = float(agg_attn['B'][i, 0, view_index])

            # Align attention matrices so that rows correspond to A-fragments and cols to B-fragments
            La = int(va_i.sum().item())
            Lb = int(vb_i.sum().item())

            def _align_ab(mat, La_, Lb_):
                if mat is None or (not hasattr(mat, "shape")):
                    return None
                if mat.shape == (La_, Lb_):
                    return mat
                if mat.shape == (Lb_, La_):
                    return mat.T
                # best-effort crop to expected shape
                return mat[:La_, :Lb_]

            A2B_ab = _align_ab(A2B_2d, La, Lb)
            B2A_ab = _align_ab(B2A_2d, La, Lb)

            def _topk_edges_dir(W, k, direc):
                if W is None or W.size == 0:
                    return []
                flat = np.argsort(-np.abs(W.ravel()))[:k]
                edges = []
                for idxv in flat:
                    ia, jb = divmod(int(idxv), W.shape[1])
                    edges.append((direc, int(ia), int(jb), float(W[ia, jb])))
                return edges

            edges_a2b = _topk_edges_dir(A2B_ab, export_top_edges_k, "A->B")
            edges_b2a = _topk_edges_dir(B2A_ab, export_top_edges_k, "B->A")
            edges_all = sorted(edges_a2b + edges_b2a, key=lambda x: -abs(x[3]))[:export_top_edges_k]

            def _frag_desc(mol, atom_list):
                return describe_fragment(mol, atom_list) if mol and atom_list else {"name": "", "smiles": ""}

            # Top fragment importance (aggregate absolute attention)
            top_frags_A, top_frags_B = [], []
            try:
                if A2B_ab is not None and B2A_ab is not None:
                    impA = np.sum(np.abs(A2B_ab), axis=1) + np.sum(np.abs(B2A_ab), axis=1)
                    impB = np.sum(np.abs(A2B_ab), axis=0) + np.sum(np.abs(B2A_ab), axis=0)
                elif A2B_ab is not None:
                    impA = np.sum(np.abs(A2B_ab), axis=1)
                    impB = np.sum(np.abs(A2B_ab), axis=0)
                elif B2A_ab is not None:
                    impA = np.sum(np.abs(B2A_ab), axis=1)
                    impB = np.sum(np.abs(B2A_ab), axis=0)
                else:
                    impA = impB = None
                if impA is not None:
                    for ia in np.argsort(-impA)[:export_top_frag_k]:
                        ia = int(ia)
                        atoms = frag2atoms_a[view_key].get(ia, [])
                        top_frags_A.append({"fragA_id": ia, "score": float(impA[ia]), "fragA": _frag_desc(mol_a, atoms)})
                if impB is not None:
                    for jb in np.argsort(-impB)[:export_top_frag_k]:
                        jb = int(jb)
                        atoms = frag2atoms_b[view_key].get(jb, [])
                        top_frags_B.append({"fragB_id": jb, "score": float(impB[jb]), "fragB": _frag_desc(mol_b, atoms)})
            except Exception:
                pass

            # Prompt-friendly top pairs (subset of top edges)
            top_pairs = edges_all[:export_top_pairs_k]
            vdict = {
                "view": view_key,
                "weightA": weightA,
                "weightB": weightB,
                "top_pairs": [],
                "top_edges": [{"dir": d, "fragA_id": ia, "fragB_id": jb, "attn": sc} for (d, ia, jb, sc) in edges_all],
                "top_fragA": top_frags_A,
                "top_fragB": top_frags_B,
            }
            for (direc, ia, jb, score) in top_pairs:
                a_atoms = frag2atoms_a[view_key].get(int(ia), [])
                b_atoms = frag2atoms_b[view_key].get(int(jb), [])
                vdict["top_pairs"].append({
                    "dir": direc, "fragA_id": int(ia), "fragB_id": int(jb),
                    "attn": float(score),
                    "fragA": _frag_desc(mol_a, a_atoms),
                    "fragB": _frag_desc(mol_b, b_atoms),
                })
        ...
        # -----------------------------
        # Minimal text mode (for paper-ready figures)
        # -----------------------------
        if minimal_text is None:
            # default to minimal for publication; set env FSE_MINIMAL_TEXT=0 to restore verbose overlays
            minimal_text = (os.environ.get('FSE_MINIMAL_TEXT', '1').strip() != '0')
        ...
        fig.legend(handles=legend_elems, loc="lower right", fontsize=12, frameon=False)

        # 标注 y 与 ŷ
        # In minimal_text mode, we avoid large title boxes inside each single-view figure,
        # because the paper grid will provide unified headers/annotations.
        try:
            if sample_dict.get('views'):
                is_class = (output[i].dim() > 0 and output[i].shape[-1] > 1)
            if is_class:
                label_str = 'Synergy' if y_true > 0.5 else 'Non-Synergy'
                title_text = f"True: {label_str} | P(Syn)={y_pred:.3f} | |err|={abs_err:.3f}"
            else:
                title_text = f"y={y_true:.2f}  ŷ={y_pred:.2f}  |err|={abs_err:.2f}"
            if minimal_text:
                fig.text(0.5, 0.97, title_text, ha='center', va='top', fontsize=15)
            else:
                fig.text(
                    0.5, 0.97, title_text, ha='center', va='top', fontsize=15,
                    bbox=dict(boxstyle='round,pad=0.45', facecolor='white', alpha=0.95,
                              edgecolor='#1f77b4', linewidth=1.6)
                )
            ...
            if not minimal_text:
                fig.text(
                    ...
                    bbox=dict(boxstyle='round,pad=0.45', facecolor='white', alpha=0.95,
                              edgecolor='#ff7f0e', linewidth=1.2)
                )
        ...
        if with_cell:
            if not minimal_text:
                fig.suptitle(f"Drug {ida}  ⇄  Drug {idb} with Cell Line {idc}", fontsize=15, y=0.99)
            base_name = f"Drug{ida}_x_Drug{idb}_with_Cell{idc}_{idx[i]}"
        else:
            if not minimal_text:
                fig.suptitle(f"Drug {ida}  ⇄  Drug {idb} without Cell Line {idc}", fontsize=15, y=0.99)
            base_name = f"Drug{ida}_x_Drug{idb}_without_Cell{idc}_{idx[i]}"

        fig_out_path = os.path.join(out_dir, f"{base_name}.png")
        # Higher DPI helps when figures are later downscaled in two-column PDFs
        _dpi = 300 if minimal_text else 200
        fig.savefig(fig_out_path, dpi=_dpi, bbox_inches="tight")
        plt.close(fig)

        # === Structured export for case study (always useful, even if call_llm=False) ===
        sample_dict["with_cell"] = bool(with_cell)
        sample_dict["figure_path"] = fig_out_path
        sample_dict["base_name"] = base_name
        if write_sample_json:
            try:
                with open(os.path.join(out_dir, f"{base_name}.{sample_json_suffix}"), "w", encoding="utf-8") as f:
                    json.dump(sample_dict, f, ensure_ascii=False, indent=2)
            except Exception as _e:
                print(f"[Warn] Failed to write sample json for {base_name}: {type(_e).__name__}: {_e}")
    ...
    out_ae = np.asarray(group_ae, dtype=np.float16)
    out_re = np.asarray(group_re, dtype=np.float16)
    if return_samples:
        return out_ae, out_re, (samples_out if samples_out is not None else [])
    return out_ae, out_re
...
def save_group_montage_by_pair(
        drug_id_a, drug_id_b, cell_id, idx, group_ae, group_re,
        out_dir, with_cell, y_true=None,  # 建议传入真实的标签数组
        base_height=2400, col_width=1600, dpi=200
):
    """
    完善后的组图拼接函数：支持分类准确率统计与回归误差统计。
    """
    pair2items = defaultdict(list)
    # 如果 y_true 为 None，我们通过对比 ae 和 re 是否相等来启发式判断分类任务
    y_true_vals = y_true if y_true is not None else [None] * len(idx)

    for ida, idb, cid, i, ae, re, yt in zip(drug_id_a, drug_id_b, cell_id, idx, group_ae, group_re, y_true_vals):
        tag = "with" if with_cell else "without"
        fname = f"Drug{ida}_x_Drug{idb}_{tag}_Cell{cid}_{i}.png"
        fpath = os.path.join(out_dir, fname)
        pair2items[(str(ida), str(idb))].append({
            "cid": cid, "idx": i, "path": fpath, "ae": ae, "re": re, "yt": yt
        })

    for (ida, idb), items in pair2items.items():
        items = [item for item in items if os.path.exists(item["path"])]
        if not items: continue
        items.sort(key=lambda x: x["idx"])

        # --- 绘图布局代码 (保持原样) ---
        cols = len(items)
        fig = plt.figure(figsize=(cols * col_width / dpi, base_height / dpi), dpi=dpi)

        list_ae = [it["ae"] for it in items]
        list_re = [it["re"] for it in items]
        list_yt = [it["yt"] for it in items]

        # --- 核心完善部分：动态判定指标 ---
        # 判定逻辑：如果传入的 yt 全是 0/1，或者是之前设置的分类标志 (ae == re)
        is_class = False
        if all(y in [0, 1] for y in list_yt if y is not None):
            is_class = True
        elif np.allclose(list_ae, list_re):  # 启发式判定
            is_class = True

        if is_class:
            # 分类指标：计算该组药物对在不同细胞系下的预测准确率
            # 在分类中，ae < 0.5 意味着预测概率偏向正确标签的一侧
            correct_count = sum(1 for ae in list_ae if ae < 0.5)
            group_acc = correct_count / len(list_ae)
            mean_gap = np.mean(list_ae)
            subtitle = f"Group Accuracy: {group_acc:.1%} | Mean Prob Gap: {mean_gap:.4f}"
        else:
            # 回归指标
            subtitle = f"Mean |err|: {np.mean(list_ae):.3f} | Mean |rel_err|: {np.mean(list_re):.3f}"

        # --- 拼接图片与渲染标题 ---
        for r, item in enumerate(items):
            ax = fig.add_axes([r / cols, 0.0, 1 / cols, 0.92])
            ax.set_axis_off()
            ax.imshow(plt.imread(item["path"]))

        full_title = f"Drug Synergy Analysis: {ida} + {idb}\n{subtitle}"
        fig.suptitle(full_title, fontsize=22, y=0.98, fontweight='bold')

        out_name = f"Pair_{ida}_x_{idb}_GROUP_{'with' if with_cell else 'without'}_Cell.png"
        # Higher DPI helps when figures are later downscaled in two-column PDFs
        _dpi = 300
